...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Seller
      .findOne(_.pick(params, _.keys(Seller.schema.paths)))
      .populate(populate);
  },

  /**
   * Promise to count sellers.
   *
   * @return {Promise}
   */
  ...
  add: async (values) => {
    ...
    const entry = await Seller.create(data);

    // Create relational data and return the entry.
    return Seller.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Seller.associations.map(a => a.alias));
    const data = _.omit(values, Seller.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Seller.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Seller.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Seller.attributes).reduce((acc, curr) => {
      switch (Seller.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
